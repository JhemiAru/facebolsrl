        ...
        <div class="card shadow-lg border-0 rounded-4 animate__animated animate__fadeIn">
            ...
            <div class="card-header rounded-top-4 p-4"
                ...
                <h5 class="mb-0 text-white fw-bold d-flex align-items-center">
                    <i class="fas fa-building me-2"></i> Adicionar Categoria
                </h5>
            </div>

            <!-- Cuerpo del formulario -->
            <div class="card-body bg-white p-5 rounded-bottom-4">
                <form action="{{ route('categorias.store') }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate id="empresaForm">
                    @csrf

                    <div class="row g-4">
                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-dark">Nombre de Categoria</label>
                            <input type="text" name="nombre" class="form-control rounded-pill shadow-sm"
                                   value="{{ old('nombre', $categoria->nombre) }}" style="text-transform: uppercase;" required>
                            @error("nombre") <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-dark">Descripción</label>
                            <input type="text" name="descripcion" class="form-control rounded-pill shadow-sm"
                                   value="{{ old('descripcion', $categoria->descripcion) }}" style="text-transform: uppercase;" required>
