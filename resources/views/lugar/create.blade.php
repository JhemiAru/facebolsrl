        ...
        <div class="card shadow-lg border-0 rounded-4 animate__animated animate__fadeIn">
            ...
            <div class="card-header rounded-top-4 p-4"
                ...
                <h5 class="mb-0 text-white fw-bold d-flex align-items-center">
                    <i class="fas fa-building me-2"></i> Adicionar Lugar
                </h5>
            </div>

            <!-- Cuerpo del formulario -->
            <div class="card-body bg-white p-5 rounded-bottom-4">
                <form action="{{ route('lugar.store') }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate id="empresaForm">
                    @csrf

                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-dark">Nombre de la Ciudad</label>
                            <input type="text" name="ciudad" class="form-control rounded-pill shadow-sm"
                                   value="{{ old('ciudad', $lugar->ciudad ?? '') }}" style="text-transform: uppercase;" required>
                            @error("ciudad") <small class="text-danger">{{ $message }}</small> @enderror
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-dark">Nombre de la Departemento</label>
                            <input type="text" name="departamento" class="form-control rounded-pill shadow-sm"
                                   value="{{ old('departamento', $lugar->departamento ?? '') }}" style="text-transform: uppercase;" required>
                            @error("departamento") <small class="text-danger">{{ $message }}</small> @enderror
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-semibold text-dark">Nombre de la Provincia</label>
                            <input type="text" name="provincia" class="form-control rounded-pill shadow-sm"
                                   value="{{ old('provincia', $lugar->provincia ?? '') }}" style="text-transform: uppercase;" required>
