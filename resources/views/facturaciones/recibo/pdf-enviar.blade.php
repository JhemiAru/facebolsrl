...
<body>
   ...
   <!-- ===== IMAGEN COMO MARCA DE AGUA ===== -->
    <img src="{{ $pdfLogoPath ?? '' }}" class="watermark-logo" alt="Marca de agua">
  
    @php
        // Función para convertir fecha a formato español en mayúsculas
        function fechaEnEspanol($fecha)
        {
            ...
            $carbon = \Carbon\Carbon::parse($fechaActual);
            $dia = $carbon->day;
        ...
        $facturasPorCliente = $facturas->groupBy('id_informacion');
    @endphp

    @foreach ($facturasPorCliente as $clienteId => $facturasCliente)
        @php
            $primeraFactura = $facturasCliente->first();
            // Recolectar todos los conceptos de todas las facturas del cliente
            $todosConceptos = collect();
            foreach ($facturasCliente as $factura) {
                if ($factura->recibo && $factura->recibo->conceptos) {
                    foreach ($factura->recibo->conceptos as $concepto) {
                        $todosConceptos->push($concepto);
                    }
                }
            }
            // Ordenar conceptos por fecha
            $todosConceptos = $todosConceptos->sortBy('fecha_concepto');
            $totalGeneral = $todosConceptos->sum('monto');
        @endphp

        <!-- Header + Información del cliente en UN solo recuadro -->
            ...
            <table class="header-table">
                <tr>
                    ...
                    <td class="text-right" style="width: 20%; font-size: 7pt;">
                        ...
                        <b>N°:</b> {{ $primeraFactura->recibo->n_recibo ?? 'N/A' }}<br>
                        <div class="estado-badge">
                            {{ $primeraFactura->estado === 'pago_deposito' ? 'DEPÓSITO' : ($primeraFactura->estado === 'pago_efectivo' ? 'EFECTIVO' : 'OTRO') }}
                        </div>
                    </td>
                </tr>
            </table>

            <!-- Información del cliente (fecha y señor) -->
            <table class="info-table">
                <tr>
                    <td colspan="2">
                        <strong>EL ALTO </strong> {{ fechaEnEspanol(now()) }}
                    </td>
                </tr>
                <tr>
                    <td style="width: 70%;">
                        ...
                        @if ($primeraFactura->informacion)
                            {{ strtoupper($primeraFactura->informacion->nombre ?? '') }}
                            {{ strtoupper($primeraFactura->informacion->apellido_paterno ?? '') }}
                            {{ strtoupper($primeraFactura->informacion->apellido_materno ?? '') }}
                        @else
                            SIN CLIENTE
                        @endif
                    </td>
                    <td style="width: 30%;">
                        ...
                        {{ $primeraFactura->ci_nit ?? 'N/A' }}
                    </td>
                </tr>
            </table>
        </div>

        @if ($primeraFactura->anulado)
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach ($todosConceptos as $concepto)
                    <tr>
                        <td style="width: 15%;">
        ...
        <!-- Monto literal en recuadro separado -->
